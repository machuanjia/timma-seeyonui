...
V3X.prototype.init = function(contextPath, language){	
	...
	this.loadLanguage("/scriptes/i18n");
	//注销office license.js 暂时无用
...
Attachment.prototype.toString = function(isShowLink, isShowDelete, onlineEdit, width){
	...
	if(isShowLink && (this.type == 0 || this.type == 3 || this.type == 5)){//downloadURL
	    if(this.type == 3){
			...
			str += "<a href=\"" + contextPath + "/fileUpload.do?method=download&fileId=" + this.fileUrl + "&createDate=" + this.createDate.substring(0, 10) + "&filename=" + encodeURIComponent(this.filename) + "\" title=\"" + escapeStringToHTML(this.filename) + "\" target='downloadFileFrame' style='font-size:12px;color:#007CD2;'>";
	    }
	}
	if(onlineEdit){
	...
	if((this.type == 2 || this.type == 4) && this.description && (onlineEdit != false)){	//文档
		...
		if(this.mimeType == "collaboration"){
			click = "openDetail('', 'from=Done&affairId=" + this.description + "&isQuote=true&openFrom=glwd&baseObjectId=" + this.reference +"&baseApp="+ this.category + suffix + "')";
		}
 ...
 * 添加附件
 */	
function addAttachment(type, filename, mimeType, createDate, size, fileUrl, canDelete, needClone, description, extension, icon, reference, category, onlineView, width) {
	...
	var attachment = new Attachment('', reference, '', category, type, filename, mimeType, createDate, size, fileUrl, description, needClone,extension,icon, onlineView);
...
function whenstart(contextPath, whoClick, myLeft, myTop, type, allowEmpty, width, height){
	...
	var rv = v3x.openWindow({
        ...
        url: contextPath + "/seeyon.ui.calendar/date.htm?type=" + type + "&allowEmpty=" + allowEmpty,
        height: 230,
        width: 250,
        'top': evt.screenY + 20,
        left: evt.screenX - 50
