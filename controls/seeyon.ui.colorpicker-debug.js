...
(function ($, undefined) {
    ...
    $.widget("evol.colorpicker", {
        ...
        options: {
            ...
            history: true,
			position:'absolute',
            strings: $.i18n('colorsPicker.Theme.js')+','+$.i18n('colorsPicker.Standard.js')+','+$.i18n('colorsPicker.More.js')+','+$.i18n('colorsPicker.Less.js')+','+$.i18n('colorsPicker.Palette.js')+','+$.i18n('colorsPicker.History.js')+','+$.i18n('colorsPicker.NoHistory.js')//'Theme Colors,Standard Colors,More Colors,Less Colors,Back to Palette,History,No history yet.'
        },
            ...
            switch (this.element.get(0).tagName) {
                case 'INPUT':
					...
					if(this.options.position == 'absolute'){
						...
						var _www = this.element.width();
	                    e.width(_www-35).addClass('left colorPicker ' + this._id)
	                        .wrap('<div style="width:' + (_www) + 'px;'
	                            + (isMoz ? 'padding:1px 0;' : '')
	                            + '"></div>')
	                        .after('<div class="evo-colorind' + (isMoz ? '-ff' : _ie) + '" ' +
	                            (color != null ? 'style="background-color:' + color + '"' : '') + ' style="float:left;position: static;"></div>')
	                        .on('keyup onpaste', function (evt) {
	                            var c = $(this).val();
	                            if (c != that.options.color) {
	                                that._setValue(c, true);
        ...
        _paletteHTML: function (json) {
            ...
            var _cssLeftHtml;
            if (_json.left != null) {
                _cssLeftHtml = "left:" + _json.left + "px;";
            }
            var _cssTopHtml;
            if (_json.top != null) {
                _cssTopHtml = "top:" + (parseInt(_json.top) - 2) + "px;";
                //_cssTopHtml = "top:" + (parseInt(_json.top) + 23) + "px;";
            }
            ...
            h.push('<div class="evo-pop', _ie, ' ui-widget ui-widget-content ui-corner-all"',
                this._isPopup ? ' style="position:'+opts.position+';' + _cssLeftHtml + _cssTopHtml + '"' : '', '>');
            // palette
            h.push('<span>', this['_paletteHTML' + pIdx](), '</span>');
            // links
            h.push('<div class="evo-more"><a href="javascript:void(0)">', labels[1 + pIdx], '</a>');
            if (opts.history) {
                //h.push('<a href="javascript:void(0)" class="evo-hist">', labels[5], '</a>');//OA-45190，交互太复杂，去掉历史功能
            }
            h.push('</div>');
            // indicator
        ...
        _paletteHTML1: function () {
            var h = [], labels = this.options.strings.split(','),
                ...
                cTD = isIE ? '"><div style="width:2px;height:2px;overflow:hidden;"></div></td>' : '"><span/></td>',
            ...
            if (this._enabled) {
                ...
                if (this._palette == null) {
                    ...
                    this._palette = this.element.next()
                        .after(this._paletteHTML({ left: this.element.offset().left, top: this.element.offset().top })).next()
