...
//	var contextPath = v3x.baseURL;
	...
	str += "<div id='attachmentDiv_" + this.fileUrl + "' class='attachment_block' style='float:"+ df +formVisible+" line-height: 14px;' noWrap>";
	if(this.type != 1 && !this.isShowImg){
		str += '<span class="ico16 '+this.icon.substring(0,this.icon.indexOf('.'))+'_16 margin_r_5"></span>'  ;
//    str += "<img src='" + contextPath + "/common/images/attachmentICON/" + this.icon + "' border='0' height='16' width='16' align='absmiddle' style='margin-right: 3px;'>";
	...
	if(this.isShowImg){
	    str += "<img style=\"CURSOR: pointer\" onclick=\"window.showModalDialog($(this).attr('src'),window,'dialogHeight:768px;dialogWidth:1024px;center:yes;resizable:yes;')\" src=\""+contextPath+"/fileUpload.do?method=showRTE&fileId="+this.fileUrl+"&createDate=" + this.createDate.substring(0, 10) + "&type=image\">"
...
/*		if(this.type == 4 || this.type == 3){
		 ...
		 str += 	'<span class="ico16 affix_del_16" onclick="deleteAttachment(\''+ this.fileUrl + '\')" ></span>';       
	}
	...
	if( seeyonProductId>0 && this.hasSaved && this.canFavourite && this.type != 2&&!this.isShowImg){
	    str += 	'<span class="ico16 collection_16" title="'+$.i18n("doc.jsp.knowledge.collection")+'" onclick="favoriteAttr(\''+ this.category + '\', \''+ this.fileUrl + '\')" ></span>';       
	}
 ...
 * 附件收藏
 */
function favoriteAttr(appName, id){
    var hasAtts=false;
    var type=4; // 附件收藏：4
 ...
 * 添加附件
 */	
function addAttachment(type, filename, mimeType, createDate, size, fileUrl, canDelete, needClone, description, extension, icon, reference, category, onlineView, width,hasSaved,isCanTransform,v,canFavourite) {
...
//					var delImg = "<img src='" + contextPath + "/common/images/attachmentICON/delete.gif' onclick='deleteAttachment(\"" + att.fileUrl + "\")' class='cursor-hand' title='" +$.i18n('common.toolbar.delete.label') + "' height='11' align='absmiddle'>";
					var delImg ='<span class="ico16 affix_del_16" onclick="deleteAttachment(\''+ att.fileUrl + '\')" ></span>';  
			...
			if(attDiv){
				...
				if(a){
//					var delImg = "<img src='" + contextPath + "/common/images/attachmentICON/delete.gif' onclick='deleteAttachment(\"" + att.fileUrl + "\")' class='cursor-hand' title='" +$.i18n('common.toolbar.delete.label') + "' height='11' align='absmiddle'>";
				  var delImg ='<span class="ico16 affix_del_16" onclick="deleteAttachment(\''+ att.filename + '\')" ></span>';  
					a[0].insertAdjacentHTML("afterEnd",delImg);	
				}
			}
		}
...
function pigeonhole(appName, ids, atts, validAcl,type){
	...
	var returnval = v3x.openWindow({
		url : _ctxPath + "/doc.do?method=docPigeonhole&appName=" + appName + "&atts=" + atts + "&validAcl=" + validAcl+"&pigeonholeType="+type,
		width : "500",
		height : "500",
		resizable : "true",
		scrollbars : "true"			
...
MxtWindow.prototype.isModel = function(){
	if(this.model){
		...
		styleStr += (this.isIE) ? "filter:alpha(opacity=4);" : "opacity:0.2;";
...
MxtWindow.prototype.getPanel = function(){
    ...
    if (this.isIframe) {
    	...
    	var ifram = this.targetWindow.document.createElement( this.isIE ? "<iframe name=\"" + this.iframeId + "\">" : "iframe"); 
...
// 打开正文编辑器插入的关联
function openEditorAssociate(url){
  window.open(url);
}
