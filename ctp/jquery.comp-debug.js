...
(function($) {
  ...
  function showBreadcrumb(div, options) {
    ...
    if (menus.length > 0) {
      ...
      for ( var i = 0; i < menus.length; i++) {
        if(menus[i].url){
          items.push('<a href="###" onclick="showMenu(\'' + _ctxPath+menus[i].url + '\')">' + menus[i].name + '</a>');
        }else{
          items.push('<a style="color:#888;" class="color_gray2_nohover">' + menus[i].name + '</a>');
        }
      }
      ;
      html += items
  ...
  $.fn.chooseProject = function(jsonObj) {
      ...
      if(htmlStr.css("width")=="100%"){
          input.css("width","100%");
          input.width(input.width()-25);
      }else{
          if(htmlStr.width()>0){
              input.width(htmlStr.width());
          }
      }
    ...
    if(options.extendWidth){
        if(valueInput.css("width")=="100%"){
            ...
            if(valueInput.width()>0){
                if (showbtn){
                    input.width(valueInput.width() - delSize);
                }
            }
        }
    }
          ...
          if(tj.showButton == true){
              ...
              if(tj.enabled===1){
                  button.unbind("click").bind("click",function(){
                      handWrite(tj.recordId,tj.signObj);
                    });
              }
              t.after(button);
              twidth = twidth-button.width();
          }
      ...
      for ( var i = 0; i < atts.length; i++) {
        ...
        if (tj.attachmentTrId){
          ...
          addAttachmentPoi(att.type, att.filename, att.mimeType,
              ...
              att.reference, att.category, false,null,embedInput,true, att.officeTransformEnable, att.v, canFavourite,isShowImg);
        }else
